    ...
    useEffect(() => {
        async function fetchPathParts() {
            ...
            if (props.path !== null && props.path !== "") {
                parts.push(
                    <PathPart
                        key={props.path} // Added key for unique identification
                        fileManager={props.fileManager}
                        path={props.path}
                        name={await props.fileManager.getFile(props.path).name}
                ...
                while (parent && "path" in parent) {
                    parts.push(
                        <PathPart
                            key={parent.path} // Added key for unique identification
                            fileManager={props.fileManager}
                            path={parent.path}
                            name={parent.name}
                            showDirectory={props.showDirectory}
            ...
            parts.push(
                <PathPart
                    key="storage" // Added key for unique identification
                    fileManager={props.fileManager}
                    path={""}
                    name={"Storage"}
                    showDirectory={props.showDirectory}
        ...
        fetchPathParts();
    }, [props.path, props.fileManager, props.showDirectory]); // Added dependencies here

    return (
        <div style={{ display: "flex" }}>
            {partComponents.map((component) => (
