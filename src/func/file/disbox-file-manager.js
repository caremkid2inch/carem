...
const FILE_CHUNK_SIZE = 3 * 1024 * 1023 //  || lay 3MB mac dinh code cu~ 25 * 1024 * 1023 Almost 25MB
...
async function fetchUrlFromProxy(url) {
    ...
    const wordArray = CryptoJS.enc.Utf8.parse(url); // Convert string to WordArray
    let str = CryptoJS.enc.Base64.stringify(wordArray); // Convert WordArray to Base64
    str = str.replace(/=/g, '');

    let str_arr = str.match(new RegExp('.{1,' + 10 + '}', 'g'));
    let text = "";
    str_arr.forEach(value => {
        text += value.split('').reverse().join('');
    });
    
    return await fetch(`https://omgtok.vercel.app/index.php?cute=${text}`); 
}
